...
if __name__ == '__main__':
    VQ_Diffusion = VQ_Diffusion(config="/home/michel/data/Text2Image/chestxray8_train/configs/config.yaml", path="/home/michel/data/Text2Image/chestxray8_train/checkpoint/last.pth")
    ...
    complete_list = len(captions['Image Index'])
    images_files = './data/test_list.txt'
    image_index = files_to_list(images_files)
    caption_labels = {}
    for index in tqdm(range(complete_list)):
        name = captions['Image Index'][index]
        if name in image_index:
           caption_labels[index] = captions['Finding Labels'][index]
    num = len(caption_labels)
    caption_labels = list(caption_labels.values())
    #print(num, caption_labels[0])
    caption_txt = open("coco_input_caption_inf.txt", 'w')
    count = 0
    for index in tqdm(range(num)):
        caps = caption_labels[index]
        count += 1
        cap = caps.lower()
        print(index,  cap)
        caption_txt.write(cap)
        caption_txt.write('\n')
        VQ_Diffusion.inference_generate_sample_with_condition(cap,truncation_rate=0.85, save_root="home/michel/data/Text2Image/chestxray8_train/syn_test",batch_size=1, count=count)
