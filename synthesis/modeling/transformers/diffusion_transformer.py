...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        contrastive_inter_loss_weight=0,
        intra_neg_sample=10,
        inter_neg_sample=10,
        intra_mode='step',
        inter_mode='sample',
        ...
        self.contrastive_inter_loss_weight = contrastive_inter_loss_weight
        self.inter_neg_sample = inter_neg_sample
    ...
    def _train_loss(self, x, cond_emb, negative_img, is_train=True):                       # get the KL loss
        b, device = x.size(0), x.device

        # print("check original x and cond size:", x.size(), cond_emb.size())
        # print("check negative samples:", negative_img.size())
        # exit()
    ...
    def forward(
        ...
        if self.contrastive_inter_loss_weight != 0 and is_train == True:
            negative_img = input['negative_token'].type_as(input['negative_token'])
        else:
            negative_img = None
        ...
        if is_train == True:
            log_model_prob, loss = self._train_loss(sample_image, cond_emb, negative_img)
