...
class Solver(object):
    def __init__(self, config, args, model, dataloader, logger):
        ...
        self.music_dir = os.path.join(args.save_dir, 'music')
    ...
    def sample_music(self, batch, phase='train', step_type='iteration', test_id=0):
        tic = time.time()
        self.logger.log_info('Begin to sample...')
        if self.ema is not None:
            self.ema.modify_to_inference()
        ...
        with torch.no_grad(): 
            ...
            step = self.last_iter if step_type == 'iteration' else self.last_epoch
            for k, v in samples.items():
                save_dir = os.path.join(self.music_dir, phase, k)
                os.makedirs(save_dir, exist_ok=True)
                save_path = os.path.join(save_dir, 'e{:010d}_itr{:010d}_rank{}{}'.format(self.last_epoch, self.last_iter%self.dataloader['train_iterations'], get_rank(), suffix))
                print("Solver sample_music:", k, v.size())
                if torch.is_tensor(v) and v.dim() == 4 and v.shape[1] in [1, 3]: # image
                    im = v
                    im = im.to(torch.uint8)
                    self.logger.add_images(tag='{}/{}e_{}itr/{}'.format(phase, self.last_epoch, self.last_iter%self.dataloader['train_iterations'], k), img_tensor=im, global_step=step, dataformats='NCHW')
                    ...
                    im_grid.save(save_path + '.jpg')
                    self.logger.log_info('save {} to {}'.format(k, save_path+'.jpg'))
                elif 'music' in k and v.dim() == 3: # may be other values, such as text caption
                    print("Check generated music sample:", v.size())
                    sample_n = v.size()[0]
                    for i in range(sample_n):
                        sample_audio = v[i,:,:]
                        if test_id == 0:
                            file_audio = save_path+str(i)+'.wav'
                        else:
                            file_audio = save_path+str(i)+'_test'+str(test_id)+'.wav'
                        sf.write(file_audio, sample_audio.squeeze().detach().cpu().numpy(), 22050)
                    # with open(save_path+'.txt', 'a') as f:
                    #     f.write(str(v)+'\n')
                    #     f.close()
                        self.logger.log_info('save {} to {}'.format(k, file_audio))
                else:
                    with open(save_path+'.txt', 'a') as f:
                        f.write(str(v)+'\n')
                        f.close()
    ...
    def train_epoch(self):
        ...
        for itr, batch in enumerate(self.dataloader['train_loader'].dataset):
            ...
            if self.sample_iterations > 0 and (self.last_iter + 1) % self.sample_iterations == 0:
                ...
                self.model.eval()
                # self.sample_music(batch, phase='train', step_type='iteration')
                # self.model.generate_content(batch=batch)
                self.model.train()
    ...
    def validate_epoch(self):
        ...
        if val:
            ...
            self.model.eval()
            # self.sample_music(batch, phase='val', step_type='iteration')
            overall_loss = None
            epoch_start = time.time()
            itr_start = time.time()
            itr = -1
            ...
            for itr, batch in enumerate(self.dataloader['validation_loader']):
                ...
                self.sample_music(batch, phase='val', step_type='iteration', test_id=test_id)
                test_id += 1
                for loss_n, loss_dict in loss.items():
